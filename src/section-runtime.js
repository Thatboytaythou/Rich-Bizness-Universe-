...
if (key === 'index' || key === 'home') {
  ...
  function label(s) { if (!s) return 'OPEN'; if (s.key === 'live') return 'WE LIT🔥'; if (s.key === 'rb-secret') return 'VAULT'; if (s.key === 'notifications') return 'ALERTS'; if (s.key === 'avatar-characters') return 'AVATARS'; return String(s.title || s.key).split(' ')[0].toUpperCase(); }
  function dock() { return `<nav class="dock"><a href="/">⌂<span>HOME</span></a>${dockKeys.map((id) => { const s = map.get(id); return s ? `<a href="${s.route}">${icons[id] || '•'}<span>${label(s)}</span></a>` : ''; }).join('')}</nav>`; }
  function profileChip() { const p = currentProfile || {}; const name = (p.display_name || p.username || 'Profile').toString(); const img = p.avatar_url ? `<img src="${p.avatar_url}" alt="">` : 'RB'; return `<a class="profile-chip" href="/profile.html"><span>${img}</span><b>${name}</b><small>${p.rank_title || 'BIZ LEGEND'}</small></a>`; }
  ...
  async function guard() { if (publicRoutes.has(key)) return null; const { session, user } = await getAuthoritativeIdentity(); if (!session || !user) { location.replace(`/auth.html?next=${encodeURIComponent(location.pathname)}`); return null; } currentUser = user; currentProfile = await ensureProfile(user); const route = profileRoute(currentProfile); if (!onboardingRoutes.has(key) && route === '/avatar.html') { location.replace('/avatar.html'); return null; } return { user, profile: currentProfile }; }
