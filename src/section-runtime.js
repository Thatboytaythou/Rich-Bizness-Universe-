...
const fallback = {
  auth: ['Tap In', 'profiles'],
  avatar: ['Avatar', 'meta_avatars'],
  edit: ['Edit Profile', 'profiles'],
  settings: ['Settings', 'user_settings'],
  search: ['Search', 'profiles'],
  watch: ['Watch', 'live_streams'],
  secret: ['Secret Door', 'profiles']
};
...
function ensure() {
  ...
  if (!$('#sectionCards')) {
    ...
    hero.insertAdjacentHTML('afterend', `<section class="layout"><section class="panel"><h2>${title}</h2><div id="sectionCards" class="cards"><div class="empty">Loading...</div></div></section><aside class="panel" id="schemaPanel"><h2>Schema Sync</h2></aside></section>`);
  }
}
...
async function rows(table) {
  try {
    const res = await supabase.from(table).select('*').limit(10);
    return res.data || [];
  } catch (_) { return []; }
}

function card(row) {
  const name = row.title || row.name || row.display_name || row.username || row.status || 'Rich Bizness Record';
  const note = row.description || row.bio || row.caption || row.created_at || 'Synced record';
  return `<article class="card"><b>${name}</b><p>${note}</p><small>${primary}</small></article>`;
}
