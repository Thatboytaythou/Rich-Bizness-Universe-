...
(() => {
  ...
  const VERSION = 'index-owned-1';
  const addCss = (href, id) => {
    let link = id ? document.getElementById(id) : null;
    if (!link) { link = document.createElement('link'); link.rel = 'stylesheet'; if (id) link.id = id; document.head.appendChild(link); }
    link.href = href;
  };
  const addModule = (src, key) => {
    if (document.querySelector(`script[data-rb-${key}]`)) return;
    const s = document.createElement('script'); s.type = 'module'; s.src = src; s.dataset[`rb${key[0].toUpperCase()}${key.slice(1)}`] = 'true'; document.head.appendChild(s);
  };
  ...
  const forceLayout = () => {
    ...
    const meta = document.querySelector('.district.meta'); if (meta) meta.style.gridColumn = '1 / -1';
  ...
  document.querySelectorAll('[data-route]').forEach((button) => {
    ...
    button.addEventListener('click', () => {
      ...
      window.setTimeout(() => { window.location.href = routeFor(key); }, 180);
    });
  });
})();
