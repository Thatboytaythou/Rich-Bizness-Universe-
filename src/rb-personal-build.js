...
css('/src/rb-personal-build.css?v=personal-3', 'rbPersonalBuildCss');
css('/src/rb-personality-motion.css?v=motion-2', 'rbPersonalityMotionCss');
...
const PAGE = {
  messages: ['RICH-DMS', 'Smoke-cloud threads, calls, reactions, reads, and typing.', 'DM STYLE', 'rolling smoke...'],
  ...
  feed: ['FEED', 'Posts, likes, comments, follows, and creator drops.', 'LATEST DROPS', 'real feed only'],
  upload: ['UPLOAD ZONE', 'Route every file to the right Rich Bizness section.', 'DROP ROUTER', 'real uploads only'],
  meta: ['META WORLD', 'Avatars, worlds, rooms, portals, inventory, chat, and stream links.', 'META MODE', 'free roam ready'],
  secret: ['SECRET DOOR', 'Private Rich Bizness access route.', 'SPECIAL ACCESS', 'gated route']
};
...
function paintPage() {
  ...
  soft('.hero h1,#sectionTitle', c[0]); soft('.hero p,#sectionSubtitle', c[1]); soft('.kicker,#sectionKicker', c[2]);
...
function stripOldDebugWords() {
  const replace = {
    'Schema Sync':'Next Action','Gaming Sync':'Next Action','Message Sync':'Next Action','Store Sync':'Next Action','Upload Sync':'Next Action','Meta Sync':'Next Action','Access Sync':'Next Action','Edit Sync':'Next Action','Settings Sync':'Next Action',
    'PRIMARY':'SECTION','PRIMARY TABLE':'SECTION','TABLES':'STATUS','SYNC':'0','Rookie Builder':'BIZ LEGEND','Tap In':'Rich Access','TAP IN':'RICH ACCESS','Universe Access':'Rich Access','Loading feed...':'Checking real feed drops...','Loading uploads...':'Checking real uploads...','Loading meta...':'Checking real meta worlds...','Loading access route...':'Checking real access...'
  };
  document.querySelectorAll('h1,h2,h3,p,small,b,span,button,a,div').forEach((el) => { if (el.children.length) return; const t = el.textContent?.trim(); if (replace[t]) el.textContent = replace[t]; });
  document.querySelectorAll('#miniProfile,#composerPanel').forEach((el) => { el.style.display = 'none'; });
}
