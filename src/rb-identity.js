...
export const DEFAULT_PROFILE = Object.freeze({
  display_name: 'ThatboyTayThou', username: 'thatboytaythou', rich_level: 1, rich_points: 0, rank_title: 'BIZ LEGEND', balance_cents: 0, avatar_url: '', banner_url: '/images/hero-banner.png', bio: 'Building my Rich Bizness lane across live, music, gaming, sports, gallery, store, meta, and money.', onboarding_state: 'needs_avatar', has_avatar: false, has_profile_identity: true, last_route: '/',
});

export function slugName(value) { return String(value || 'thatboytaythou').toLowerCase().replace(/[^a-z0-9_]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 28) || 'thatboytaythou'; }
...
export async function getSessionUser() {
  try { return (await getAuthoritativeIdentity()).user; } catch (_) { return null; }
}
