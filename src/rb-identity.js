...
export const DEFAULT_PROFILE = Object.freeze({
  display_name: 'Rich Bizness User', username: 'rich_user', rich_level: 1, rich_points: 0, rank_title: 'BIZ LEGEND', balance_cents: 0, avatar_url: '', banner_url: '', bio: 'Building a Rich Bizness lane across the universe.', onboarding_state: 'needs_avatar', has_avatar: false, has_profile_identity: true, last_route: '/', secret_access_level: 'locked', vault_unlocked: false, secret_room_keys: [], last_secret_route: '', rb_secret_rank: 'LOCKED',
});

export const DEFAULT_USER_SETTINGS = Object.freeze({
  language: 'en', timezone: '', default_theme: 'smoke-cloud', accent_color: '#9dff63', cinema_mode: true, tv_mode: true, motion_level: 'full', profile_visibility: 'public', dm_privacy: 'everyone', notification_level: 'all', metadata: { default_upload_route: 'feed', favorite_start_route: '/', rb_language: 'Settings Hub', source: 'tap-in-foundation' },
});
...
export function clearIdentityCache() { identityCache = null; identityCacheAt = 0; }
...
function secretPatch(profile = {}) { const points = Number(profile.rich_points || 0); const creator = !!(profile.is_creator || profile.is_seller || profile.is_artist); const current = profile.secret_access_level || 'locked'; const level = current !== 'locked' ? current : points >= 2500 ? 'elite' : points >= 500 ? 'member' : creator ? 'creator' : 'locked'; const rank = points >= 5000 ? 'MYTHIC VAULT' : points >= 2500 ? 'ELITE DOOR' : points >= 500 ? 'SECRET MEMBER' : creator ? 'CREATOR DOOR' : 'LOCKED'; return { vault_unlocked: Boolean(profile.vault_unlocked || points >= 500 || creator), secret_access_level: level, rb_secret_rank: profile.rb_secret_rank && profile.rb_secret_rank !== 'LOCKED' ? profile.rb_secret_rank : rank, secret_room_keys: Array.isArray(profile.secret_room_keys) ? profile.secret_room_keys : [] }; }
...
export async function getAuthoritativeIdentity(options = {}) {
  ...
  identityFlight = (async () => {
    ...
    if (!user) {
      identityCache = { session: null, user: null, profile: null };
...
export async function ensureProfile(user) {
  ...
  const row = { id: user.id, username: slugName(seed), display_name: display, full_name: display, avatar_url: meta.avatar_url || '', banner_url: '', bio: DEFAULT_PROFILE.bio, rich_level: 1, rich_points: 0, rank_title: 'BIZ LEGEND', balance_cents: 0, online_status: 'online', onboarding_state: 'needs_avatar', has_avatar: false, has_profile_identity: true, last_route: '/avatar.html', secret_access_level: 'locked', vault_unlocked: false, secret_room_keys: [], rb_secret_rank: 'LOCKED', metadata: { rb_language: 'Tap In', source: 'global_tap_in_foundation' } };
  let result = await supabase.from('profiles').upsert(row, { onConflict: 'id' }).select().maybeSingle();
  if (result.error && String(result.error.message || '').toLowerCase().includes('username')) result = await supabase.from('profiles').upsert({ ...row, username: uniqueUsername(seed, user.id) }, { onConflict: 'id' }).select().maybeSingle();
