...
function svg(){
  ...
  const smoke = pick('smoke') === 'none' ? '' : `<path d="M72 108c78 34 107-34 175-4 66 30 110-22 193 14" stroke="${g}" stroke-width="9" fill="none" opacity=".25"/><path d="M78 522c80-42 115 26 178-10 70-40 110 16 180-22" stroke="${g}" stroke-width="9" fill="none" opacity=".25"/>`;
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 640"><circle cx="256" cy="320" r="230" fill="${g}" opacity=".10"/>${smoke}<ellipse cx="256" cy="570" rx="126" ry="24" fill="${g}" opacity=".18"/><circle cx="256" cy="156" r="54" fill="#7a3d18" stroke="${o}" stroke-width="8"/><path d="M205 116c28-34 76-34 102 0" stroke="#090603" stroke-width="26" stroke-linecap="round"/>${hat}<path d="M256 214v168" stroke="${g}" stroke-width="24" stroke-linecap="round"/><path d="M158 268l98 56 98-56" stroke="${g}" stroke-width="22" stroke-linecap="round" stroke-linejoin="round" fill="none"/><path d="M256 382l-72 150M256 382l72 150" stroke="${g}" stroke-width="24" stroke-linecap="round"/><path d="M148 550h86M278 550h86" stroke="${o}" stroke-width="17" stroke-linecap="round"/>${chain}<text x="256" y="341" text-anchor="middle" fill="${o}" font-family="Impact,Arial Black" font-size="48">${initials(name)}</text></svg>`)}`;
}
function render(){ if (!body) return; let img = body.querySelector('img'); if (!img) { body.textContent=''; img=document.createElement('img'); img.className='avatar-preview-img'; body.appendChild(img); } img.src = svg(); body.dataset.motion = motionInput?.value || 'Boss Idle'; if (status) status.textContent = 'Stick figure layer active.'; }
