...
function avatarSvg(name, cfg) {
  ...
  const torso = cfg.gender === 'girl'
    ? 'M184 318c12-64 36-91 72-91s60 27 72 91l38 119H146z'
    : 'M164 318c12-70 45-96 92-96s80 26 92 96v105c0 31-26 49-92 49s-92-18-92-49z';
  const hairPath = cfg.hair === 'longFlow'
    ? `<path d="M188 143c14-60 120-63 136 0 16 63-2 111-31 148 7-64-1-104-37-104s-44 40-37 104c-29-37-47-85-31-148z" fill="${hair}"/>`
    : cfg.hair === 'braids'
      ? `<path d="M185 172c-20 49-18 100-4 143M327 172c20 49 18 100 4 143" stroke="${hair}" stroke-width="18" stroke-linecap="round"/>`
      : cfg.hair === 'afro'
        ? `<circle cx="256" cy="146" r="82" fill="${hair}"/>`
        : `<path d="M196 147c24-48 99-52 120-4-32-15-76-16-120 4z" fill="${hair}"/>`;
...
function paint() {
  ...
  img.src = avatarSvg(displayName.value || profile?.display_name || 'RB', cfg);
  body.style.backgroundImage = 'none';
