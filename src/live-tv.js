...
async function auth() { const state = await getAuthoritativeIdentity({ fresh: true }); user = state.user; profile = state.profile || null; return state; }
...
function idleScreen() { return `<div class="holo-empty"><div class="holo-orb">RB</div><b>${LIVE_TITLE} READY</b><p>${LIVE_ROOM} • camera + mic ready</p></div>`; }
...
document.addEventListener('visibilitychange', () => { if (document.hidden && live) forceStopMedia(); });
