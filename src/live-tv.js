...
async function endLive() { try { if (stream?.id || user?.id) await postJson('/api/livekit-stream', { action: 'end', userId: user?.id, room: stream?.livekit_room_name || roomName() }).catch(() => {}); forceStopMedia(); setStatus('READY'); paint(); } catch (err) { fail('END LIVE FAILED', err); } }
