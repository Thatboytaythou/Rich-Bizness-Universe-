...
function setStatus(text) { const el = $('#liveStudioStatus') || $('#tvStatus'); if (el) el.textContent = text; const state = $('#tvState'); if (state) state.textContent = text.includes('WE LIT') ? LIVE_TITLE : text.includes('ready') || text.includes('Ready') || text.includes('GET RIGHT') ? 'READY' : 'WORKING'; }
...
async function goLive() { try { const state = await auth(); if (!state.session || !user) { location.href = '/auth.html?next=' + encodeURIComponent('/live.html'); return; } if (!localTracks.length) await preview(); setStatus('WE LIT🔥'); const lkRoom = roomName(); const { data, error } = await supabase.functions.invoke('livekit-token', { body: { room: lkRoom, role: 'host', identity: user.id, name: user.email || 'Rich Bizness Host' } }); if (error) throw error; if (!data?.token) throw new Error(data?.error || 'LiveKit token missing'); room = new Room({ adaptiveStream: true, dynacast: true }); registerMedia(); await room.connect(data.livekitUrl || LIVEKIT_URL, data.token); for (const track of localTracks) await room.localParticipant.publishTrack(track); registerMedia(); const row = { host_id: user.id, title: LIVE_TITLE, display_room_name: LIVE_ROOM, livekit_room_name: lkRoom, status: 'live', status_label: LIVE_TITLE, viewer_count: 0, total_chat_messages: 0, metadata: { brand: 'Rich Bizness', screen: 'live' } }; const upsert = await supabase.from('live_streams').upsert(row, { onConflict: 'host_id' }).select().maybeSingle(); if (upsert.error) throw upsert.error; stream = upsert.data; live = true; setStatus(LIVE_TITLE); paint(); await awardXp('live_started', { section: 'live', sourceTable: 'live_streams', sourceId: stream?.id }).catch(() => {}); } catch (err) { setStatus(String(err.message || err)); console.warn(err); } }
