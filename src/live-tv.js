...
function stopCamera() { localTracks.forEach((t) => { try { t.stop(); } catch (_) {} }); localTracks = []; document.querySelectorAll('video').forEach((video) => { if (!video.closest('#holoScreen')) { try { video.pause(); } catch (_) {} try { video.srcObject?.getTracks?.().forEach((track) => track.stop()); } catch (_) {} video.remove(); } }); previewing = false; }
function attachVideo(track) { const screen = $('#holoScreen'); if (!screen || !track) return; screen.innerHTML = ''; const el = track.attach(); el.autoplay = true; el.muted = true; el.playsInline = true; el.style.width = '100%'; el.style.height = '100%'; el.style.objectFit = 'cover'; el.style.borderRadius = '24px'; screen.appendChild(el); }
...
async function endLive() { try { live = false; stopCamera(); if (room) room.disconnect(); room = null; if (stream?.id) await supabase.from('live_streams').update({ status: 'ended', ended_at: new Date().toISOString() }).eq('id', stream.id); setStatus('READY'); paint(); } catch (err) { setStatus(String(err.message || err)); } }
...
$('#watchXp')?.addEventListener('click', () => { stopCamera(); location.href = '/watch.html'; });
$('#imOut')?.addEventListener('click', () => { stopCamera(); location.href = '/'; });
