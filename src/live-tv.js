...
async function postJson(url, payload) { const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload || {}) }); const data = await res.json().catch(() => ({})); if (!res.ok || data?.ok === false) throw new Error(data?.error || `${url} failed`); return data; }
...
async function logEvent(type, payload = {}) { if (!stream?.id) return; await postJson('/api/live-room-event', { room: stream.livekit_room_name, stream_id: stream.id, event_type: type, identity: user?.id || null, name: profile?.display_name || user?.email || null, user_id: user?.id || null, payload }).catch(() => {}); }
async function saveMember(status = 'active') { if (!stream?.id || !user?.id) return; await postJson('/api/live-member', { stream_id: stream.id, user_id: user.id, role: 'host', status, metadata: { source: 'live-studio', display_name: profile?.display_name || null, username: profile?.username || null } }).catch(() => {}); }
