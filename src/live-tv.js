...
function safeRoomName(value = 'Bizness Party') {
  return String(value).replace(/[^a-zA-Z0-9_-]/g, '-').slice(0, 80) || 'Bizness-Party';
}
function roomName() { return stream?.livekit_room_name || 'bizness-party'; }
function setStatus(text) { const el = $('#liveStudioStatus'); if (el) el.textContent = text; }
...
function paint() {
  if (!previewing && !live) $('#holoScreen').innerHTML = idleScreen();
  $('#tvTitle').textContent = stream?.title || 'We 🔥 📺';
  $('#tvRoom').textContent = stream?.livekit_room_name || 'Bizness Party';
...
async function previewCamera() {
  ...
  try {
    ...
    setStatus('Camera blocked: allow camera and mic');
    console.warn(err);
  }
}
async function tokenFor(role, lkRoom) {
  ...
  const res = await fetch('https://xfsrqomsiulswbalgknx.supabase.co/functions/v1/livekit-token', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    body: JSON.stringify({ room: lkRoom, role, identity: user.id, name: user.email || 'Rich Bizness Host' })
  });
...
async function ensureStream() {
  ...
  if (stream?.id && stream.status === 'live') return stream;
  const lkRoom = safeRoomName('bizness-party-' + user.id.slice(0, 8));
  const { data, error } = await supabase.from('live_streams').insert({ creator_id: user.id, title: 'We 🔥 📺', display_slug: 'We 🔥 📺', display_room_name: 'Bizness Party', livekit_room_name: lkRoom, status: 'live', status_label: 'Get Right', started_at: new Date().toISOString(), metadata: { livekit_url: LIVEKIT_URL, studio: 'we_fire_tv' } }).select('*').single();
...
async function goLive() {
  try {
    ...
    previewing = false;
    setStatus('LIVE NOW 🔴');
    await supabase.from('live_stream_members').insert({ stream_id: row.id, user_id: user.id, role: 'host', status: 'active' });
...
async function endLive() {
  try {
    ...
    live = false;
    previewing = false;
    if (stream?.id) await supabase.from('live_streams').update({ status: 'ended', ended_at: new Date().toISOString() }).eq('id', stream.id);
...
async function watching() {
  if (!stream?.id) return;
  await getUser();
  await supabase.from('live_view_sessions').insert({ stream_id: stream.id, user_id: user?.id || null, metadata: { screen: 'we_fire_tv', livekit_url: LIVEKIT_URL } });
