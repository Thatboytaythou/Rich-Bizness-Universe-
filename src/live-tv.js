...
async function getLiveKitToken(payload) {
  const res = await fetch('/api/livekit-token', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  const data = await res.json().catch(() => ({}));
  if (!res.ok || !data?.token || !data?.livekitUrl) throw new Error(data?.error || 'Vercel LiveKit token failed');
  return data;
}
...
async function goLive() { try { const state = await auth(); if (!state.session || !user) { location.href = '/auth.html?next=' + encodeURIComponent('/live.html'); return; } if (!localTracks.length) await preview(); setStatus(LIVE_TITLE); const lkRoom = roomName(); const lk = await getLiveKitToken({ room: lkRoom, role: 'host', identity: user.id, name: user.email || 'Rich Bizness Host' }); room = new Room({ adaptiveStream: true, dynacast: true }); registerMedia(); await room.connect(lk.livekitUrl, lk.token); for (const track of localTracks) await room.localParticipant.publishTrack(track); registerMedia(); const row = { creator_id: user.id, slug: lkRoom, display_slug: lkRoom, title: LIVE_TITLE, display_room_name: LIVE_ROOM, livekit_room_name: lkRoom, status: 'live', status_label: LIVE_TITLE, access_type: 'free', price_cents: 0, currency: 'usd', viewer_count: 0, total_chat_messages: 0, is_chat_enabled: true, is_cohost_enabled: true, metadata: { brand: 'Rich Bizness', screen: 'live', token_source: 'vercel' } }; const upsert = await supabase.from('live_streams').upsert(row, { onConflict: 'slug' }).select().maybeSingle(); if (upsert.error) throw upsert.error; stream = upsert.data; live = true; setStatus(LIVE_TITLE); paint(); await awardXp('live_started', { section: 'live', sourceTable: 'live_streams', sourceId: stream?.id }).catch(() => {}); } catch (err) { setStatus(String(err.message || err)); console.warn(err); } }
async function endLive() { try { if (stream?.id) await supabase.from('live_streams').update({ status: 'ended', status_label: 'ROOM ENDED', ended_at: new Date().toISOString() }).eq('id', stream.id); forceStopMedia(); setStatus('READY'); paint(); } catch (err) { setStatus(String(err.message || err)); } }
