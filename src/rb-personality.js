...
const FALLBACK = {
  ...
  home_labels: { welcome: 'WELCOME BACK', rich_level: 'RICH LEVEL', rank_default: 'BIZ LEGEND', online_label: 'ONLINE', portal: 'PORTAL HUB', live: 'WE LIT🔥', live_room: 'Bizness Party', profile: 'XP WALLET' },
  live_labels: { screen: 'WE LIT🔥', room: 'Bizness Party', status: 'Get Right', cohost: 'Rich Playa Co-Host Room', party_over: 'Party Over', chat_enabled: 'Chat enabled', replay_enabled: 'Replay enabled' },
...
const PAGE_COPY = {
  ...
  live: ['WE LIT🔥', 'Live studio, broadcast room, chat, replay, and creator XP.', 'GO LIVE'],
...
export async function loadPersonality() { try { const { data, error } = await supabase.rpc('rb_personality', { p_key: null }); if (!error && data && typeof data === 'object') cache = { ...FALLBACK, ...data }; } catch (_) {} applyPersonality(cache); return cache; }
...
function applyPageCopy() { const key = pageKey(); const copy = PAGE_COPY[key]; if (!copy) return; soft('.hero h1,#sectionTitle', copy[0]); soft('.hero p,#sectionSubtitle', copy[1]); soft('.panel h2', key === 'messages' ? 'Rich Threads' : key === 'gaming' ? 'Elite Game Hub' : copy[0]); document.title = `${copy[0]} - Rich Bizness`; document.body.dataset.rbPageReady = 'true'; }
function cleanOldWords() { const swaps = new Map([['WE FIRE TV','WE LIT🔥'],['Schema Sync','Next Action'],['Gaming Sync','Next Action'],['Message Sync','Next Action'],['Store Sync','Next Action'],['Upload Sync','Next Action'],['Meta Sync','Next Action'],['Access Sync','Next Action'],['PRIMARY','SECTION'],['PRIMARY TABLE','SECTION'],['TABLES','STATUS'],['SYNC','0'],['Tap In','Rich Access'],['TAP IN','RICH ACCESS'],['Rookie Builder','BIZ LEGEND'],['Live 24/7','Live from records'],['ENERGY HIGH','REAL DATA']]); document.querySelectorAll('h1,h2,h3,p,small,b,span,button,a,div').forEach((el) => { if (el.children.length) return; const t = el.textContent?.trim(); if (swaps.has(t)) el.textContent = swaps.get(t); }); }
function ensureMotionLayer() { if (document.getElementById('rbMotionLayer')) return; const layer = document.createElement('div'); layer.id = 'rbMotionLayer'; layer.setAttribute('aria-hidden', 'true'); layer.innerHTML = '<i></i><i></i><i></i>'; document.body.appendChild(layer); }
export function applyPersonality(p = cache) { const voice = p.brand_voice || FALLBACK.brand_voice; const home = p.home_labels || FALLBACK.home_labels; const live = p.live_labels || FALLBACK.live_labels; const dm = p.dm_labels || FALLBACK.dm_labels; document.documentElement.dataset.rbPersonality = 'thatboytaythou'; document.documentElement.dataset.rbTheme = p.visual_identity?.theme || 'smoke-cloud'; document.body?.classList.add('rb-personality-on'); set('[data-rb-brand]', voice.brand); set('[data-rb-app]', voice.app); set('[data-rb-owner]', voice.owner_handle); set('[data-rb-motto]', voice.motto); set('[data-rb-welcome]', home.welcome); set('[data-rb-rank-default]', home.rank_default); soft('[data-rb-live-title], #tvTitle', live.screen); soft('[data-rb-live-room], #tvRoom', live.room); set('[data-rb-dm-brand]', dm.brand); applyPageCopy(); cleanOldWords(); ensureMotionLayer(); }
loadPersonality();
