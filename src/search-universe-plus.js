...
function resetLinks() { const box = $('#rbWebLinks'); if (box) box.innerHTML = '<span>Rich Bizness results only</span>'; }
...
async function runPlus() { const q = cleanTerm($('#rbSearchInput')?.value); resetLinks(); const out = $('#rbResults'); if (!out || !q) return; out.innerHTML = '<div class="rb-search-empty">Searching Rich Bizness lanes...</div>'; const rows = (await Promise.all(lanes.map(l=>laneSearch(l, `%${q}%`)))).flat(); out.innerHTML = rows.length ? rows.map(card).join('') : '<div class="rb-search-empty">No Rich Bizness results yet.</div>'; const count = $('#recordCount'); if (count) count.textContent = String(rows.length); const status = $('#tableCount'); if (status) status.textContent = 'LIVE'; const primary = $('#primaryTable'); if (primary) primary.textContent = 'Universe Search'; }
function boot() { if (window.RB_SEARCH_UNIVERSE_BOOTED) return; window.RB_SEARCH_UNIVERSE_BOOTED = true; resetLinks(); $('#rbSearchButton')?.addEventListener('click', runPlus); $('#rbSearchInput')?.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); runPlus(); } }); }
boot();