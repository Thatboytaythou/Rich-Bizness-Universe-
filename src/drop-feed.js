...
import { getAuthoritativeIdentity, ensureProfile } from './rb-identity.js?v=identity-public-2';
...
function feedRow({ title, caption, upload }) {
  return {
    ...
    thumbnail_url: upload.mediaType === 'image' ? upload.publicUrl : '',
    section: selectedRoute === 'gallery' ? 'gallery' : selectedRoute === 'feed' ? 'feed' : selectedRoute,
    visibility: 'public',
    post_type: selectedRoute,
    file_url: upload.publicUrl,
    cover_url: upload.mediaType === 'image' ? upload.publicUrl : '',
    metadata: identityMeta({ public_url: upload.publicUrl, bucket: upload.bucket, file_path: upload.filePath }),
  };
}
...
function uploadLogRow({ title, caption, upload }) {
  return {
    ...
    media_type: upload.mediaType,
    visibility: 'public',
    processing_status: upload.publicUrl ? 'uploaded' : 'text-only',
    metadata: identityMeta({ route_table: routeInfo()[1] }),
  };
}
...
function sectionRows({ title, caption, upload, feedPostId }) {
  ...
  if (selectedRoute === 'sports') return [
    ['sports_uploads', { ...id, title, caption, sport_name: 'Rich Bizness Sports', content_type: upload.mediaType, clip_type: 'drop', file_url: upload.publicUrl, thumbnail_url: upload.mediaType === 'image' ? upload.publicUrl : '', metadata: meta }],
    ['sports_posts', { ...id, title, body: caption, sport: 'Rich Bizness Sports', media_url: upload.publicUrl, media_type: upload.mediaType, cover_url: upload.mediaType === 'image' ? upload.publicUrl : '', thumbnail_url: upload.mediaType === 'image' ? upload.publicUrl : '', metadata: meta }],
  ];
  if (selectedRoute === 'gaming') return [
    ['gaming_uploads', { ...id, title, description: caption, caption, category: 'clip', media_type: upload.mediaType, file_url: upload.publicUrl, clip_url: upload.publicUrl, thumbnail_url: upload.mediaType === 'image' ? upload.publicUrl : '', bucket: upload.bucket, file_path: upload.filePath, visibility: 'public', processing_status: 'uploaded', metadata: meta }],
    ['game_clips', { ...id, title, caption, clip_url: upload.publicUrl, thumbnail_url: upload.mediaType === 'image' ? upload.publicUrl : '', feed_post_id: feedPostId || null, metadata: meta }],
  ];
...
async function submitUpload(e) {
  ...
  try {
    ...
    const caption = $('#dropCaption')?.value?.trim() || 'They’re here Rich';
    ...
    console.warn('[RB upload] blocked', error);
    $('#dropStatus').textContent = error.message || 'Drop blocked by table policy.';
    $('#dfStatusTop').textContent = 'BLOCKED';
  }
}
...
async function submitFeed(e) {
  ...
  const body = $('#feedBody')?.value?.trim() || 'They’re here Rich';
  ...
  const row = {
    ...
    post_type: 'feed',
    media_url: media,
    media_type: media ? 'link' : 'text',
    file_url: media,
    visibility: 'public',
    metadata: identityMeta({ media_url: media }),
  };
...
function renderFeed() {
  ...
  box.innerHTML = posts.length ? posts.map((p) => {
    ...
    return `<article class="df-post"><div class="df-author"><div class="df-face">${face ? `<img src="${face}" alt="">` : 'RB'}</div><div><b>${authorOf(p)}</b><small>${p.section || 'feed'} • ${new Date(p.created_at || Date.now()).toLocaleString()}</small></div></div><h3>${nameOf(p)}</h3><p>${p.body || p.caption || p.description || 'They’re here Rich'}</p>${mediaHtml ? `<div class="df-media">${mediaHtml}</div>` : ''}<div class="df-post-actions"><button type="button">💨 Like</button><button type="button">Comment</button><button type="button">Share</button></div></article>`;
...
async function loadFeed() {
  try {
    ...
    if ($('#dfStatusTop')) $('#dfStatusTop').textContent = 'READY';
  }
}
