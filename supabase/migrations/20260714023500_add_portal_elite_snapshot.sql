create or replace function public.rb_portal_elite_snapshot()
returns jsonb
language sql
security definer
...
settings as (select to_jsonb(s.*) j from public.user_settings s,me where s.user_id=me.uid),
...
unread_threads as (
  ...
  where m.status='active' and coalesce(t.is_archived,false)=false
  and (m.last_read_at is null or t.last_message_at>m.last_read_at)
),
announcement as (
...
background as (
  select to_jsonb(b.*) j from public.background_presets b where b.is_active=true
  order by b.is_premium desc,b.updated_at desc limit 1
),
layout as (
  select to_jsonb(l.*) j from public.layout_presets l where l.is_active=true and l.section in ('portal','global')
  order by l.is_premium desc,l.updated_at desc limit 1
),
pulse as (
...
recent as (
  select coalesce(jsonb_agg(row_data order by created_at desc),'[]'::jsonb) j from (
    select jsonb_build_object('kind','live','title',coalesce(title,'Live room'),'created_at',created_at,'href','/live.html') row_data,created_at from public.live_streams
    union all
    select jsonb_build_object('kind','music','title',coalesce(title,'Music drop'),'created_at',created_at,'href','/music.html'),created_at from public.music_tracks
    union all
    select jsonb_build_object('kind','store','title',coalesce(title,'Store drop'),'created_at',created_at,'href','/store.html'),created_at from public.products where coalesce(is_public,true)=true
    union all
    select jsonb_build_object('kind','sports','title',coalesce(title,'Sports post'),'created_at',created_at,'href','/sports.html'),created_at from public.sports_posts
    union all
    select jsonb_build_object('kind','meta','title',coalesce(title,'Meta world'),'created_at',created_at,'href','/meta.html'),created_at from public.meta_worlds where lower(coalesce(status,'active')) not in ('deleted','hidden')
...
select jsonb_build_object(
  ...
  'settings',coalesce((select j from settings),'{}'::jsonb),
...
$function$;
revoke all on function public.rb_portal_elite_snapshot() from public,anon;
