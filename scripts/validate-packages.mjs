...
const xpSource = readFileSync(resolve(root, 'packages/config/src/xp.ts'), 'utf8');
for (const status of ['queued', 'processing', 'awarded', 'rejected', 'failed']) {
  if (!xpSource.includes(`${status}: '${status}'`)) throw new Error(`Missing canonical XP queue status: ${status}`);
}
if (/processed:\s*'processed'/.test(xpSource)) throw new Error('XP success state must be awarded, not processed.');
if (!xpSource.includes('XP_PER_LEVEL = 1000')) throw new Error('XP level contract must use the canonical 1000 XP progression step.');
...
console.log(`Package validation passed: ${Object.keys(exportsMap).length} config exports, ${slugs.length} game contracts, 5 XP states, 6 workspace aliases.`);