...
export default async function handler(req, res) {
  ...
  const livekitUrl = process.env.LIVEKIT_URL || 'wss://rich-bizness-mobile-app-ww6cieid.livekit.cloud';
  if (!key || !secret) return send(res, 500, { ok: false, error: 'LiveKit env missing' });
  const body = req.body || {};
  const role = body.role === 'host' ? 'host' : 'audience';
  const room = clean(body.room, 'we-lit-room');
