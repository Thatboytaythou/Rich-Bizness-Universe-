...
type XpSnapshot = {
  ...
  rich_points?: number;
  coins?: number;
  recent?: Array<{ event_key?: string; xp?: number; created_at?: string }>;
};
...
export async function mountXpRuntime(): Promise<void> {
  ...
  if (immersivePages.has(section) || document.querySelector('#rbXpDock')) return;
  ...
  const render = (data: XpSnapshot | null) => {
    ...
    panel.querySelector<HTMLElement>('#rbXpRecent')!.innerHTML = recent.length
      ? recent.map((row) => `<article><span>${esc(String(row.event_key ?? 'XP EARNED').replaceAll('_', ' ').toUpperCase())}</span><strong>+${Number(row.xp ?? 0)} XP</strong></article>`).join('')
      : '<p>Your XP activity will appear here.</p>';
  };
  ...
  const load = (): Promise<void> => {
    ...
    loadPromise = (async () => {
      do {
        ...
        const { data } = await supabase.rpc('rb_xp_snapshot', { p_user_id: session.user.id });
        render(data as XpSnapshot | null);
  ...
  await supabase.rpc('rb_award_xp', {
    p_event_key: 'daily_tap_in', p_section: 'auth', p_source_table: null, p_source_id: null, p_amount: null
  });
  if (!['tap-in', 'auth'].includes(section)) {
    await supabase.rpc('rb_award_xp', {
      p_event_key: 'section_visit', p_section: section, p_source_table: null, p_source_id: null, p_amount: null
    });
  }
  await load();

  const channel = supabase
    .channel(`xp-runtime:${session.user.id}`)
    .on('postgres_changes', {
      event: '*', schema: 'public', table: 'user_levels', filter: `user_id=eq.${session.user.id}`
    }, () => { void load(); })
    .subscribe();
