...
const safe = (value: any) => { try { const url = new URL(String(value || ''), location.origin); return ['http:', 'https:'].includes(url.protocol) ? url.href : ''; } catch { return ''; } };
...
export async function mount(): Promise<void> {
  ...
  let active = episodes.find((episode) => String(episode.id) === requested) ?? episodes[0] ?? null;
  let showId: any = active?.show_id ?? shows[0]?.id ?? 'all';
  let channel: any = null;
  let historyTimer = 0;

  root.innerHTML = `<main class="media-ultimate podcast-universe"><div class="media-ultimate__wrap"><header class="media-ultimate__head"><a href="/music.html">←</a><div><p>RICH BIZNESS ORIGINAL AUDIO</p><h1>PODCAST UNIVERSE</h1></div><nav class="media-ultimate__tabs"><a href="/music.html">MUSIC</a><a class="active" href="/podcast.html">PODCAST</a><a href="/radio.html">RADIO</a></nav></header><section class="podcast-stage"><article id="podcastCover" class="podcast-cover">${active ? cover(active, liked.has(String(active.id))) : '<div class="media-ultimate__empty">No podcast episode released.</div>'}</article><aside class="podcast-queue"><section class="media-ultimate__metrics"><article><small>SHOWS</small><strong>${shows.length}</strong></article><article><small>EPISODES</small><strong>${episodes.length}</strong></article><article><small>TOTAL PLAYS</small><strong>${fmt(episodes.reduce((n, e) => n + Number(e.play_count ?? 0), 0))}</strong></article><article><small>COMMUNITY</small><strong>${fmt(episodes.reduce((n, e) => n + Number(e.comment_count ?? 0), 0))}</strong></article></section><nav class="media-ultimate__tabs" id="showTabs"><button class="${showId === 'all' ? 'active' : ''}" data-show="all">ALL SHOWS</button>${shows.map((s) => `<button class="${String(showId) === String(s.id) ? 'active' : ''}" data-show="${s.id}">${esc(s.title)}</button>`).join('')}</nav><div id="episodeQueue"></div></aside></section><section class="media-ultimate__split"><article class="media-ultimate__panel"><header><h4>EPISODE NOTES & PERFORMANCE</h4></header><div id="episodeDetail" class="media-ultimate__list"></div></article><article class="media-ultimate__panel"><header><h4>LISTENER CONVERSATION</h4></header><div id="podcastComments" class="media-ultimate__chat"></div><form id="podcastCommentForm" class="media-ultimate__form"><input id="podcastCommentInput" maxlength="2000" placeholder="Join the episode discussion..."><button class="media-ultimate__btn primary">POST</button></form></article></section><aside class="universe-player" id="podcastPlayer" hidden><img id="podcastPlayerCover" alt=""><div><h3 id="podcastPlayerTitle"></h3><p id="podcastPlayerMeta"></p></div><audio id="podcastAudio" controls preload="metadata"></audio></aside></div></main>`;
  ...
  const loadComments = async () => {
    ...
    const { data: rows, error: commentsError } = await supabase.from('podcast_comments').select('id,body,display_name,username,created_at').eq('episode_id', active.id).order('created_at', { ascending: true }).limit(100);
    if (commentsError) { comments.innerHTML = `<div class="media-ultimate__empty">${esc(commentsError.message)}</div>`; return; }
    comments.innerHTML = (rows ?? []).map((c: any) => `<article><p>${esc(c.body)}</p><small>${esc(c.display_name || c.username || 'Rich Listener')}</small></article>`).join('') || '<div class="media-ultimate__empty">Start the discussion.</div>';
    comments.scrollTop = comments.scrollHeight;
  };

  const renderQueue = () => {
    queue.innerHTML = visible().map((e) => `<article class="podcast-episode ${String(active?.id) === String(e.id) ? 'active' : ''}" data-id="${e.id}"><img src="${esc(safe(e.cover_url || e.show_cover_url) || '/images/brand/IMG_5997.png')}" alt=""><div><h4>${esc(e.title || 'Rich Podcast')}</h4><p>${esc(e.show_title || e.display_name || 'Rich Original')} · S${e.season_number ?? 1} E${e.episode_number ?? '—'} · ${duration(e.duration_seconds)}</p></div><strong>${fmt(e.play_count)} ▶</strong></article>`).join('') || '<div class="media-ultimate__empty">No episodes in this show.</div>';
  ...
  const openEpisode = async (episode: Row) => {
    active = episode;
    document.querySelector<HTMLElement>('#podcastCover')!.innerHTML = cover(episode, liked.has(String(episode.id)));
    ...
    const src = safe(episode.audio_url || episode.file_url);
    if (src) {
      player.hidden = false;
      document.querySelector<HTMLImageElement>('#podcastPlayerCover')!.src = safe(episode.cover_url || episode.show_cover_url) || '/images/brand/IMG_5997.png';
  ...
  document.querySelector<HTMLFormElement>('#podcastCommentForm')!.onsubmit = async (event) => {
    ...
    const { error: commentError } = await supabase.from('podcast_comments').insert({ episode_id: active.id, user_id: session.user.id, body, username: profile.username, display_name: profile.display_name });
    if (!commentError) { input.value = ''; await loadComments(); }
  };
  audio.addEventListener('timeupdate', () => {
    if (!active || !Number.isFinite(audio.duration) || Date.now() - historyTimer < 12000) return;
    historyTimer = Date.now();
    void supabase.from('audio_listening_history').upsert({ user_id: session.user.id, source_type: 'podcast', source_id: active.id, progress_seconds: Math.floor(audio.currentTime), completed: false, last_played_at: new Date().toISOString(), metadata: { duration_seconds: Math.floor(audio.duration || 0), title: active.title } }, { onConflict: 'user_id,source_type,source_id' });
  });
  ...
  renderQueue();
  if (active) await openEpisode(active); else document.querySelector<HTMLElement>('#podcastCover')!.innerHTML = '<div class="media-ultimate__empty">No podcast episode released.</div>';
}

function cover(e: Row, isLiked: boolean) {
  return `<img src="${esc(safe(e.cover_url || e.show_cover_url) || '/images/brand/IMG_5997.png')}" alt=""><div class="podcast-cover__copy"><span class="media-ultimate__eyebrow">RICH ORIGINAL · ${esc(e.show_category || 'PODCAST')}</span><h2>${esc(e.title || 'Rich Podcast')}</h2><p>${esc(e.description || 'Long-form conversation, culture and creator stories from the Rich Bizness universe.')}</p><div class="media-ultimate__actions"><button id="podcastPlayBtn" class="media-ultimate__btn primary">▶ PLAY EPISODE</button><button id="podcastLikeBtn" class="media-ultimate__btn">${isLiked ? '♥ LIKED' : '♡ LIKE'}</button><a class="media-ultimate__btn" href="/profile.html?id=${esc(e.creator_id || e.user_id)}">HOST PROFILE</a></div></div>`;
}
