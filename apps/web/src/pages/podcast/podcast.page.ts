...
export async function mount(): Promise<void> {
  ...
  const renderQueue = () => {
    if (!isCurrent()) return;
    queue.innerHTML = visible().map((episode, index) => `<button class="podcast-episode ${String(active?.id) === String(episode.id) ? 'active' : ''}" data-id="${esc(episode.id)}"><img src="${esc(safe(episode.thumbnail_url || episode.cover_url || episode.show_cover_url) || '/images/brand/IMG_5997.png')}" alt=""><div><h4>${esc(episode.title || 'Rich Podcast')}</h4><p>${esc(episode.show_title || episode.display_name || 'Rich Original')} · ${runtime(episode.duration_seconds)} · ${esc((episode.media_type || 'audio').toUpperCase())}</p></div><strong>${String(index + 1).padStart(2, '0')}</strong></button>`).join('') || '<div class="media-ultimate__empty">No episodes in this show.</div>';
  ...
  const syncUrl = (episodeId: string) => { if (!isCurrent()) return; const url = new URL(location.href); url.searchParams.set('episode', episodeId); url.searchParams.delete('id'); history.replaceState({}, '', `${url.pathname}${url.search}${url.hash}`); };
  ...
  const load = async (episodeId?: string, open = true) => {
    ...
    try {
      ...
      const requested = episodeId || params.get('episode') || params.get('id');
