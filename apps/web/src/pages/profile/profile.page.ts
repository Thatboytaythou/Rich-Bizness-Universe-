...
export async function mountProfilePage():Promise<void>{
 ...
 const channel=supabase.channel(`profile-universe:${profileId}`).on('postgres_changes',{event:'UPDATE',schema:'public',table:'profiles',filter:`id=eq.${profileId}`},()=>location.reload()).on('postgres_changes',{event:'*',schema:'public',table:'followers'},()=>void 0).subscribe();
