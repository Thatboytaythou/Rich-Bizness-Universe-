...
export async function mount(): Promise<void> {
  ...
  const runLiveAction = async (action: string, payload: Row = {}) => {
    if (!active?.id || !requireUser()) return null;
  ...
  const hero = (row: Row) => {
    ...
    const playback=!liveNow(row)&&safeMedia(row.recording_url)?`<video class="media-ultimate__hero-media live-inline-video" controls playsinline poster="${esc(poster)}" src="${esc(safeMedia(row.recording_url))}"></video>`:`<img class="media-ultimate__hero-media" src="${esc(poster)}" alt="">`;
    const gated=['vip','paid','private'].includes(String(row.access_type));
    const host=user&&String(row.creator_id)===String(user.id);
    return `${playback}<div class="media-ultimate__hero-copy"><span class="media-ultimate__eyebrow">${liveNow(row)?'● WE LIT🔥':'WE 🔥📺'} · ${esc(row.display_room_name||row.category||'FAMILY BIZNESS')}</span><h2>${esc(row.title||'Family Bizness')}</h2><p>${esc(row.description||'Pop in and see what type time the Rich Bizness universe on.')}</p><div class="media-ultimate__actions">${liveNow(row)?`<button id="popInBtn" class="media-ultimate__btn primary">${user?'POP IN':'TAP IN TO POP IN'}</button>`:'<a class="media-ultimate__btn primary" href="/watch.html">WATCH REPLAY</a>'}${gated&&!host?`<button id="buyAccessBtn" class="media-ultimate__btn">UNLOCK ${money(row.price_cents??0)}</button>`:''}${liveNow(row)&&!host?'<button id="tipBtn" class="media-ultimate__btn">💰 TIP</button>':''}<button id="reactBtn" class="media-ultimate__btn">💨 REACT</button><button id="alertBtn" class="media-ultimate__btn">${alerts.has(String(row.creator_id))?'🔔 LOCKED IN':'🔔 STAY LOCKED'}</button><button id="shareBtn" class="media-ultimate__btn">↗ SHARE</button></div></div>`;
  };
  ...
  const renderRoom = () => {
    ...
    document.querySelector<HTMLButtonElement>('#buyAccessBtn')?.addEventListener('click',async()=>{const result=await runLiveAction('purchase_access',{});if(result){await refresh();void popIn(active!);}});
    document.querySelector<HTMLButtonElement>('#tipBtn')?.addEventListener('click',async()=>{const amount=Number(prompt('Tip amount in dollars','5')||0);if(!Number.isFinite(amount)||amount<=0)return;const result=await runLiveAction('tip',{amount_cents:Math.round(amount*100)});if(result){burst('💰');await refresh();}});
    document.querySelector<HTMLButtonElement>('#reactBtn')?.addEventListener('click',async()=>{const result=await runLiveAction('reaction',{reaction:'💨'});if(result)burst('💨');});
    document.querySelector<HTMLButtonElement>('#alertBtn')?.addEventListener('click',async()=>{const result=await runLiveAction('toggle_alert',{});if(result){const creator=String(active!.creator_id);if(result.active)alerts.add(creator);else alerts.delete(creator);renderRoom();}});
  ...
  const applySnapshot = (value: unknown) => {
    ...
    if(!active||![...streams,...recordings].some((row)=>String(row.id)===String(active?.id)))active=streams.find(liveNow)??streams[0]??recordings[0]??null;
  };
  const refresh = async () => { if(snapshotBusy){snapshotQueued=true;return;}snapshotBusy=true;do{snapshotQueued=false;const {data,error}=await supabase.rpc('rb_live_snapshot',{p_stream_id:active?.id??null});if(error){showActionError(error.message);}else if(!disposed){applySnapshot(data);renderRoom();}}while(snapshotQueued&&!disposed);snapshotBusy=false; };
  ...
  const open = async (row: Row) => {
    ...
    await refresh();
    if(liveNow(row)){
      roomChannel=supabase.channel(`live-room:${row.id}`)
  ...
  document.querySelector<HTMLFormElement>('#chatForm')!.onsubmit=async(event)=>{event.preventDefault();if(!active||!requireUser())return;const input=document.querySelector<HTMLInputElement>('#chatInput')!;const message=input.value.trim();if(!message)return;const submit=event.submitter as HTMLButtonElement|null;if(submit)submit.disabled=true;const result=await runLiveAction('chat',{message});if(result){input.value='';await refresh();}if(submit)submit.disabled=false;};
