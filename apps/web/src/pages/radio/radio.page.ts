...
export async function mount(): Promise<void> {
  ...
  let stations = (data ?? []) as Row[];
  let active: Row | null = stations[0] ?? null;
  let liked = false;
  let channel: any = null;

  root.innerHTML = `<main class="sound-universe radio-universe"><div class="sound-wrap"><header class="sound-head"><a href="/portal.html">←</a><div class="sound-brand"><small>RICH BIZNESS LIVE AUDIO</small><h1>RADIO UNIVERSE</h1></div><nav class="sound-nav"><a href="/music.html">MUSIC</a><a href="/podcast.html">PODCAST</a><a class="active" href="/radio.html">RADIO</a></nav></header><section class="sound-hero"><article id="radioHero" class="sound-now"></article><aside class="sound-panel"><section class="sound-metrics"><article><small>STATIONS</small><strong id="stationCount">${fmt(stations.length)}</strong></article><article><small>LIVE NOW</small><strong id="liveCount">${fmt(stations.filter((x) => x.is_live).length)}</strong></article><article><small>LISTENERS</small><strong id="listenerCount">${fmt(stations.reduce((n, x) => n + Number(x.listener_count ?? 0), 0))}</strong></article><article><small>LIKES</small><strong id="likeCount">${fmt(stations.reduce((n, x) => n + Number(x.like_count ?? 0), 0))}</strong></article></section><div id="radioList" class="sound-list"></div></aside></section><section class="sound-lower"><article class="sound-panel"><h3>ON AIR NETWORK</h3><div class="radio-network-copy"><p>Continuous Rich Bizness radio, creator takeovers, interviews, DJ sets and live community broadcasts connected to every Rich ID.</p><div class="radio-network-badges"><span>24/7 SIGNAL</span><span>CREATOR TAKEOVERS</span><span>LIVE DJ SETS</span><span>GLOBAL AUDIO</span></div></div></article><article class="sound-panel"><h3>STATION STATUS</h3><div id="radioStatus" class="sound-empty">Select a station to connect.</div></article></section><aside id="radioPlayer" class="sound-player" hidden><img id="radioPlayerCover" alt=""><div><strong id="radioPlayerTitle"></strong><small id="radioPlayerMeta"></small></div><audio id="radioAudio" controls autoplay preload="none"></audio></aside></div></main>`;
  ...
  const refreshMetrics = () => {
    document.querySelector<HTMLElement>('#stationCount')!.textContent = fmt(stations.length);
    document.querySelector<HTMLElement>('#liveCount')!.textContent = fmt(stations.filter((x) => x.is_live).length);
    document.querySelector<HTMLElement>('#listenerCount')!.textContent = fmt(stations.reduce((n, x) => n + Number(x.listener_count ?? 0), 0));
    document.querySelector<HTMLElement>('#likeCount')!.textContent = fmt(stations.reduce((n, x) => n + Number(x.like_count ?? 0), 0));
  };
  ...
  const renderList = () => {
    list.innerHTML = stations.map((station) => `<button class="sound-card ${active && String(active.id) === String(station.id) ? 'active' : ''}" data-id="${station.id}"><img src="${esc(safe(station.cover_url) || '/images/brand/IMG_5997.png')}" alt=""><span><b>${esc(station.station_name || station.title || 'Rich Radio')}</b><small>${station.is_live ? '● LIVE' : 'RADIO'} · ${esc(station.station_tag || station.genre || 'Global')}</small></span><strong>${fmt(station.listener_count)} ◉</strong></button>`).join('') || '<div class="sound-empty">The Radio Universe is ready for its first station.</div>';
  ...
  const openStation = async (station: Row) => {
    ...
    audio.src = src;
    status.innerHTML = src ? `<strong>${station.is_live ? 'LIVE SIGNAL' : 'STREAM READY'}</strong><br>${fmt(station.listener_count)} listeners · ${fmt(station.peak_listeners)} peak · ${esc(station.mood || station.genre || 'Global audio')}` : 'STREAM URL NOT AVAILABLE';
  ...
  const reloadStations = async () => {
    const { data: next, error: reloadError } = await supabase.from('radio_stations').select('*').eq('is_public', true).order('is_featured', { ascending: false }).order('created_at', { ascending: false }).limit(100);
    if (reloadError) return;
    stations = (next ?? []) as Row[];
    refreshMetrics();
    const current = active ? stations.find((x) => String(x.id) === String(active?.id)) : null;
    if (current) active = current;
    renderList();
  };

  renderList();
  channel = supabase.channel('rich-radio-stations').on('postgres_changes', { event: '*', schema: 'public', table: 'radio_stations' }, () => void reloadStations()).on('postgres_changes', { event: '*', schema: 'public', table: 'radio_likes' }, () => { if (active) void refreshLike().then(() => renderList()); }).subscribe();
  window.addEventListener('pagehide', () => { audio.pause(); audio.removeAttribute('src'); audio.load(); if (channel) void supabase.removeChannel(channel); }, { once: true });
