...
export async function mount(): Promise<void> {
  ...
  if (!user) { location.replace('/tap-in.html?next=%2Fcreator.html'); return; }
  ...
  root.innerHTML = `<main class="creator-shell"><div class="creator-wrap">
    ...
    <section id="creatorHero" class="creator-hero"><div class="creator-hero__copy"><span class="creator-kicker">BUILD · PUBLISH · EARN · CONTROL</span><h2 id="creatorTitle">YOUR CREATOR UNIVERSE</h2><p id="creatorTagline">Connect every section of your brand from one command center.</p><div class="creator-actions"><a class="creator-btn primary" href="/upload.html">CREATE DROP</a><a class="creator-btn" href="/live.html">GO LIVE</a><a class="creator-btn" href="/store.html">MANAGE STORE</a><a class="creator-btn" href="/creator-dimensions.html">SECRET DIMENSIONS</a><a class="creator-btn" href="/profile.html">VIEW PROFILE</a></div></div></section>
  ...
  let lane = 'dashboard';
  let profile: Row = {}, settings: Row = {}, balance: Row = {};
  let tracks: Row[] = [], products: Row[] = [], streams: Row[] = [], worlds: Row[] = [], games: Row[] = [], posts: Row[] = [];
  let alertSubscribers = 0, loading = false, queued = false, destroyed = false;
  const channels: ReturnType<typeof supabase.channel>[] = [];

  const setStatus = (message: string, error = false) => { status.textContent = message; status.dataset.error = String(error); window.setTimeout(() => { if (status.textContent === message) status.textContent = ''; }, 3000); };
  const lanes = [['dashboard', 'DASHBOARD'], ['page', 'CREATOR PAGE'], ['content', 'CONTENT'], ['earnings', 'EARNINGS'], ['alerts', 'ALERTS'], ['dimensions', 'DIMENSIONS']];
  ...
  const renderTabs = () => { tabs.innerHTML = lanes.map(([key, label]) => `<button class="creator-tab ${lane === key ? 'active' : ''}" data-lane="${key}">${label}</button>`).join(''); tabs.querySelectorAll<HTMLButtonElement>('[data-lane]').forEach((button) => { button.onclick = () => { lane = button.dataset.lane || 'dashboard'; renderTabs(); render(); }; }); };
  ...
  const render = () => {
    ...
    if (lane === 'dashboard') content.innerHTML = `<section class="creator-section"><header><div><p>CONNECTED CHANNELS</p><h2>Your active business lanes</h2></div><a class="creator-btn" href="/upload.html">NEW DROP</a></header><div class="creator-grid">
      ...
      ${card('Creator Dimensions', 'Four protected paid creator systems', '/creator-dimensions.html', profile.banner_url, 'SECRET', 'Admin-approved access')}
    </div></section>`;
    else if (lane === 'page') {
      content.innerHTML = `<section class="creator-section"><header><div><p>PUBLIC CREATOR IDENTITY</p><h2>Control your universe</h2></div></header><form id="creatorForm" class="creator-form">
        ...
        <button class="creator-btn primary" type="submit">SAVE CREATOR UNIVERSE</button></form></section>`;
      ...
      document.querySelector<HTMLFormElement>('#creatorForm')!.onsubmit = async (event) => {
        ...
        const heroBackground = document.querySelector<HTMLInputElement>('#heroBackground')!.value.trim(); const heroVideo = document.querySelector<HTMLInputElement>('#heroVideo')!.value.trim();
        ...
        const payload = { user_id: user.id, creator_title: document.querySelector<HTMLInputElement>('#creatorTitleInput')!.value.trim(), creator_tagline: document.querySelector<HTMLInputElement>('#creatorTaglineInput')!.value.trim(), hero_background_url: heroBackground || null, hero_video_url: heroVideo || null, page_theme: document.querySelector<HTMLSelectElement>('#pageTheme')!.value, intro_style: document.querySelector<HTMLSelectElement>('#introStyle')!.value, monetization_style: document.querySelector<HTMLSelectElement>('#monetizationStyle')!.value, featured_track_id: document.querySelector<HTMLSelectElement>('#featuredTrack')!.value || null, featured_product_id: document.querySelector<HTMLSelectElement>('#featuredProduct')!.value || null, featured_live_id: document.querySelector<HTMLSelectElement>('#featuredLive')!.value || null, featured_world_id: document.querySelector<HTMLSelectElement>('#featuredWorld')!.value || null, featured_game_id: document.querySelector<HTMLSelectElement>('#featuredGame')!.value || null, show_music: document.querySelector<HTMLInputElement>('#showMusic')!.checked, show_store: document.querySelector<HTMLInputElement>('#showStore')!.checked, show_live: document.querySelector<HTMLInputElement>('#showLive')!.checked, show_meta: document.querySelector<HTMLInputElement>('#showMeta')!.checked, show_gallery: document.querySelector<HTMLInputElement>('#showGallery')!.checked, show_games: document.querySelector<HTMLInputElement>('#showGames')!.checked, updated_at: new Date().toISOString() };
        setStatus('SAVING YOUR CREATOR UNIVERSE...'); const { error } = await supabase.from('creator_page_settings').upsert(payload, { onConflict: 'user_id' }); if (error) return setStatus(error.message, true);
        await supabase.from('profiles').update({ is_creator: true, updated_at: new Date().toISOString() }).eq('id', user.id); await supabase.rpc('rb_award_xp', { p_event_key: 'creator_publish', p_section: 'creator', p_source_table: 'creator_page_settings', p_source_id: null, p_amount: null }); setStatus('CREATOR UNIVERSE SAVED — XP SYNCED'); await load();
      };
    } else if (lane === 'content') {
      const rows = [...posts.map((row) => card(row.title || row.body || 'Feed Drop', row.section || row.post_type || 'feed', `/feed.html?post=${row.id}`, row.thumbnail_url || row.cover_url || row.media_url, 'FEED', `${Number(row.view_count ?? 0).toLocaleString()} views`)), ...tracks.map((row) => card(row.title || 'Music Drop', row.genre || 'music', `/music.html?track=${row.id}`, row.cover_url, 'MUSIC', `${Number(row.play_count ?? 0).toLocaleString()} plays`)), ...products.map((row) => card(row.title || 'Store Drop', money(row.price_cents, row.currency), `/store.html?product=${row.id}`, row.image_url || row.cover_url, 'STORE', `${Number(row.sales_count ?? 0).toLocaleString()} sold`)), ...streams.map((row) => card(row.title || 'Live Broadcast', row.status || 'live', `/live.html?id=${row.id}`, row.thumbnail_url || row.cover_url, 'LIVE', `${Number(row.viewer_count ?? 0).toLocaleString()} viewers`))];
    ...
    } else if (lane === 'earnings') content.innerHTML = `<section class="creator-section"><header><div><p>CREATOR WALLET</p><h2>Revenue position</h2></div></header><div class="creator-metrics"><article><small>EARNED</small><strong>${money(balance.earned_cents, balance.currency)}</strong></article><article><small>AVAILABLE</small><strong>${money(balance.available_cents, balance.currency)}</strong></article><article><small>PENDING</small><strong>${money(balance.pending_cents, balance.currency)}</strong></article><article><small>PAID OUT</small><strong>${money(balance.paid_out_cents, balance.currency)}</strong></article></div><div class="creator-actions"><a class="creator-btn primary" href="/settings.html">PAYOUT SETTINGS</a><a class="creator-btn" href="/store.html">ORDER CENTER</a></div></section>`;
    else if (lane === 'alerts') content.innerHTML = `<section class="creator-section"><header><div><p>CREATOR ALERTS</p><h2>Audience subscriptions</h2></div></header><div class="creator-panel"><div class="creator-row"><img src="${esc(safeMedia(profile.avatar_url) || '/images/brand/Avatar-hero-Banner.png.jpeg')}" alt=""><div><h4>${alertSubscribers.toLocaleString()} active alert members</h4><p>Followers can subscribe to all activity, posts, Live, Store, or Music updates.</p></div><a class="creator-btn" href="/notifications.html">OPEN</a></div></div></section>`;
  ...
  const load = async () => {
    ...
    loading = true;
    const id = user.id;
    const [profileResult, settingsResult, balanceResult, tracksResult, productsResult, streamsResult, worldsResult, postsResult, alertResult, gamesResult] = await Promise.all([
      supabase.from('profiles').select('*').eq('id', id).single(), supabase.from('creator_page_settings').select('*').eq('user_id', id).maybeSingle(), supabase.from('creator_available_balances').select('*').eq('artist_user_id', id).maybeSingle(), supabase.from('music_tracks').select('*').eq('user_id', id).order('created_at', { ascending: false }).limit(30), supabase.from('products').select('*').eq('seller_id', id).order('created_at', { ascending: false }).limit(30), supabase.from('live_streams').select('*').eq('creator_id', id).order('created_at', { ascending: false }).limit(30), supabase.from('meta_worlds').select('*').eq('owner_id', id).order('created_at', { ascending: false }).limit(30), supabase.from('feed_posts').select('*').eq('user_id', id).order('created_at', { ascending: false }).limit(30), supabase.from('creator_alert_subscriptions').select('id', { count: 'exact', head: true }).eq('creator_id', id).eq('is_active', true), supabase.from('games').select('*').eq('is_active', true).order('is_featured', { ascending: false }).limit(30)
    ]);
    loading = false;
    if (destroyed) return;
    const firstError = [profileResult, settingsResult, balanceResult, tracksResult, productsResult, streamsResult, worldsResult, postsResult, gamesResult].find((result) => result.error)?.error;
    if (firstError) return setStatus(firstError.message, true);
    profile = profileResult.data ?? {}; settings = settingsResult.data ?? {}; balance = balanceResult.data ?? {}; tracks = tracksResult.data ?? []; products = productsResult.data ?? []; streams = streamsResult.data ?? []; worlds = worldsResult.data ?? []; posts = postsResult.data ?? []; games = gamesResult.data ?? []; alertSubscribers = alertResult.count ?? 0; render();
    if (queued) { queued = false; void load(); }
  };

  renderTabs(); await load();
  ['creator_page_settings', 'creator_available_balances', 'music_tracks', 'products', 'live_streams', 'meta_worlds', 'feed_posts', 'creator_alert_subscriptions', 'creator_dimension_access'].forEach((table) => { const channel = supabase.channel(`creator:${table}:${user.id}`).on('postgres_changes', { event: '*', schema: 'public', table }, () => void load()).subscribe(); channels.push(channel); });
  const cleanup = () => { if (destroyed) return; destroyed = true; channels.forEach((channel) => void supabase.removeChannel(channel)); };
