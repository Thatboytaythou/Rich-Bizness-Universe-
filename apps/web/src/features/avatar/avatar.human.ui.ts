...
export function mountHumanUi(root:HTMLElement,data:{name:string;level:number;xp:number;rank:string;presets:Preset[];aura:string;selectedPresetKey:string}){
  ...
  root.innerHTML=`<main class="avatar-shell human-owner"><header class="avatar-top"><a href="/avatar.html" aria-label="Back to character selector">←</a><div><p>RICH BIZNESS CHARACTER UNIVERSE</p><h1>3D Avatar Lobby</h1></div><div class="avatar-top-actions"><a href="/profile.html">PROFILE</a><a href="/meta.html">META</a><div class="avatar-live"><i></i> LIVE CHARACTER</div></div></header><section class="avatar-workspace"><div class="avatar-stage"><div class="avatar-engine-bar"><span><i></i> CINEMATIC HUMAN RIG</span><div><button data-camera="orbit" class="active">ORBIT</button><button data-camera="street">STREET</button><button data-camera="portrait">PORTRAIT</button></div><button id="fullscreenBtn">FULLSCREEN</button></div><div class="avatar-motion-bar">${['idle','walk','run','combat','power','smoke','dance'].map(x=>`<button data-motion="${x}">${x.toUpperCase()}</button>`).join('')}</div><div class="avatar-scene-frame"><canvas id="avatarCanvas"></canvas><div class="avatar-scene-vignette" aria-hidden="true"></div><div class="avatar-stage-badge"><small>UNIVERSAL CHARACTER</small><strong id="stageIdentity">${esc(data.name)}</strong><span id="stagePreset"></span></div></div><div class="avatar-control-deck"><div class="avatar-hud"><div><span id="motionState">IDLE</span><b id="stageName">${esc(data.name)}</b></div><div class="avatar-meter"><span>LEVEL ${data.level}</span><span>${data.xp} XP</span><span>${esc(data.rank)}</span></div></div><div class="avatar-mobile-controls"><div id="joystick" class="avatar-joystick"><i></i></div><div><button data-action="jump">JUMP</button><button data-action="sprint">RUN</button><button data-action="power">POWER</button><button data-action="combat">COMBAT</button></div></div></div></div><aside class="avatar-console"><section class="avatar-console-card"><div class="avatar-console-title"><h3>CHARACTER</h3><button type="button" data-panel-toggle="identity" aria-expanded="true">HIDE</button></div><div class="avatar-console-body" data-panel="identity"><label>DISPLAY NAME<input id="displayName" maxlength="80" value="${esc(data.name)}"></label><div class="avatar-gender-switch" role="group" aria-label="Body type"><button type="button" data-body="male">BOY</button><button type="button" data-body="female">GIRL</button></div><div id="identityTags" class="avatar-runtime-strip"></div></div></section><section class="avatar-console-card"><div class="avatar-console-title"><h3>LOOK + MOTION</h3><button type="button" data-panel-toggle="look" aria-expanded="false">OPEN</button></div><div class="avatar-console-body is-collapsed" data-panel="look"><div class="avatar-custom-grid"><label>BUILD<select id="buildSelect"><option value="athletic">ATHLETIC</option><option value="heroic">HEROIC</option><option value="street">STREET</option></select></label><label>HAIR<select id="hairSelect"><option value="energy">ENERGY</option><option value="locs">LOCS</option><option value="long-wave">LONG WAVE</option></select></label><label>STYLE<select id="styleSelect"><option value="human rig">HUMAN RIG</option><option value="anime">ANIME</option><option value="boss">BOSS</option></select></label><label>AURA<select id="auraSelect">${['Emerald Gold','Diamond Mist','Neon Phantom'].map(x=>`<option>${x}</option>`).join('')}</select></label><label>SMOKE<select id="smokeSelect"><option value="cinematic">CINEMATIC</option><option value="heavy">HEAVY</option><option value="off">OFF</option></select></label></div></div></section><section class="avatar-console-card"><div class="avatar-console-title"><h3>PRESETS</h3><button type="button" data-panel-toggle="presets" aria-expanded="true">HIDE</button></div><div class="avatar-console-body" data-panel="presets"><div id="presetOptions" class="avatar-grid"></div></div></section><div class="avatar-save-row"><button id="resetBtn">RESET</button><button id="saveBtn" class="primary">SAVE CHARACTER</button></div><p id="status" role="status">One saved character powers Profile, Portal, Meta and free roam.</p></aside></section></main>`;
  ...
  root.querySelectorAll<HTMLButtonElement>('[data-panel-toggle]').forEach(button=>button.addEventListener('click',()=>{const key=button.dataset.panelToggle??'';const panel=root.querySelector<HTMLElement>(`[data-panel="${key}"]`);if(!panel)return;const collapsed=panel.classList.toggle('is-collapsed');button.setAttribute('aria-expanded',String(!collapsed));button.textContent=collapsed?'OPEN':'HIDE';},{signal}));
