...
export function mountHumanUi(root:HTMLElement,data:{name:string;level:number;xp:number;rank:string;presets:Preset[];aura:string;selectedPresetKey:string}){
  ...
  root.innerHTML=`<main class="avatar-shell human-owner">
    <header class="avatar-top"><a href="/avatar.html" aria-label="Back to character selector">←</a><div><p>RICH BIZNESS CHARACTER UNIVERSE</p><h1>3D Avatar Lobby</h1></div><div class="avatar-top-actions"><a href="/profile.html">PROFILE</a><a href="/meta.html">META</a><div class="avatar-live"><i></i> LIVE</div></div></header>
    <section class="avatar-workspace single-stage">
      <div class="avatar-stage">
        <div class="avatar-engine-bar"><span><i></i> LIVE 3D CHARACTER</span><div><button data-camera="orbit" class="active">ORBIT</button><button data-camera="street">STREET</button><button data-camera="portrait">PORTRAIT</button></div><button id="fullscreenBtn">FULL</button></div>
        <div class="avatar-scene-frame"><canvas id="avatarCanvas"></canvas><div class="avatar-stage-badge"><small id="motionState">IDLE</small><strong id="stageIdentity">${esc(data.name)}</strong><span id="stagePreset"></span></div></div>
        <div class="avatar-control-deck compact"><div class="avatar-action-row">${['idle','walk','run','combat','power','smoke','dance'].map(x=>`<button data-motion="${x}">${x.toUpperCase()}</button>`).join('')}</div><div class="avatar-mobile-controls"><div id="joystick" class="avatar-joystick"><i></i></div><div><button data-action="jump">JUMP</button><button data-action="sprint">RUN</button><button data-action="power">POWER</button><button data-action="combat">COMBAT</button></div></div></div>
      </div>
      <aside class="avatar-console compact-editor">
        <button id="editToggle" class="avatar-edit-toggle" type="button" aria-expanded="false">EDIT CHARACTER</button>
        <div id="editorPanel" class="avatar-editor-panel" hidden>
          <section class="avatar-console-card"><div class="avatar-console-title"><h3>CHARACTER</h3></div><div class="avatar-console-body"><label>NAME<input id="displayName" maxlength="80" value="${esc(data.name)}"></label><div class="avatar-gender-switch" role="group" aria-label="Body type"><button type="button" data-body="male">BOY</button><button type="button" data-body="female">GIRL</button></div><div id="identityTags" class="avatar-runtime-strip"></div></div></section>
          <section class="avatar-console-card"><div class="avatar-console-title"><h3>STYLE</h3></div><div class="avatar-console-body"><div class="avatar-custom-grid"><label>BUILD<select id="buildSelect"><option value="athletic">ATHLETIC</option><option value="heroic">HEROIC</option><option value="street">STREET</option></select></label><label>HAIR<select id="hairSelect"><option value="energy">ENERGY</option><option value="locs">LOCS</option><option value="long-wave">LONG WAVE</option></select></label><label>STYLE<select id="styleSelect"><option value="human rig">HUMAN</option><option value="anime">ANIME</option><option value="boss">BOSS</option></select></label><label>AURA<select id="auraSelect">${['Emerald Gold','Diamond Mist','Neon Phantom'].map(x=>`<option>${x}</option>`).join('')}</select></label><label class="avatar-smoke-field">SMOKE<select id="smokeSelect"><option value="cinematic">CINEMATIC</option><option value="heavy">HEAVY</option><option value="off">OFF</option></select></label></div></div></section>
          <section class="avatar-console-card avatar-presets-card"><div class="avatar-console-title"><h3>PRESETS</h3></div><div class="avatar-console-body"><div id="presetOptions" class="avatar-grid"></div></div></section>
          <div class="avatar-save-row"><button id="resetBtn">RESET</button><button id="saveBtn" class="primary">SAVE CHARACTER</button></div><p id="status" role="status">Saved character syncs across Rich Bizness.</p>
        </div>
      </aside>
    </section>
  </main>`;
  ...
  const editToggle=root.querySelector<HTMLButtonElement>('#editToggle')!;
  const editorPanel=root.querySelector<HTMLElement>('#editorPanel')!;
  ...
  const refresh=(preset:Preset|undefined,aura:string)=>{
    ...
    presetBox.innerHTML=data.presets.map(x=>`<button class="${x.preset_key===preset?.preset_key?'active':''}" data-preset="${esc(x.preset_key)}"><b>${esc(x.title)}</b><small>${esc(x.outfit)}</small></button>`).join('');
  ...
  editToggle.addEventListener('click',()=>{const open=editorPanel.hidden;editorPanel.hidden=!open;editToggle.setAttribute('aria-expanded',String(open));editToggle.textContent=open?'CLOSE EDITOR':'EDIT CHARACTER';},{signal});
